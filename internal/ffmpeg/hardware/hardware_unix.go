...
const (
	...
	ProbeQSVH264   = "-init_hw_device qsv -f lavfi -i testsrc2 -t 1 -vf format=nv12,hwupload -c h264_qsv -f null -"
	ProbeQSVH265   = "-init_hw_device qsv -f lavfi -i testsrc2 -t 1 -vf format=nv12,hwupload -c h265_qsv -f null -"
	ProbeQSVJPEG   = "-init_hw_device qsv -f lavfi -i testsrc2 -t 1 -vf format=nv12,hwupload -c mjpeg_qsv -f null -"
...
func ProbeAll(bin string) []*api.Source {
	...
	return []*api.Source{
		{
			Name: runToString(bin, ProbeQSVH264),
			URL:  "ffmpeg:...#video=h264#hardware=" + EngineQSV,
		},
		{
			Name: runToString(bin, ProbeQSVH265),
			URL:  "ffmpeg:...#video=h264#hardware=" + EngineQSV,
		},
		{
			Name: runToString(bin, ProbeQSVJPEG),
			URL:  "ffmpeg:...#video=h264#hardware=" + EngineQSV,
		},
		{
